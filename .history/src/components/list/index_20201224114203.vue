...
export default {
  ...
  setup(props) {
    ...
    watch(props.headers, () => { console.log(props.headers) });
    
    let __params = {};
    const request = async (params?) => {
      params && (page.current = 1, __params = params);
      loading.value = true;
      let res = await axios.post<any, AxResponse>(props.url!, { ...props.default, ...__params, current: page.current, size: page.size }, props.headers? props.headers: {});
      if (res.result) {
