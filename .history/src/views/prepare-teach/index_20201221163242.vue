<template>
  <template ref="headerRef">
    <HeaderRefComponent @type-change="typeChange" @search="params.title = $event" :searchShow='params.value' />
  </template>
  <div>
    <QueryClassComponent @query="request($event)" :searchShow='params.value'/>
    <div>
      <div class="cus-list clearfix" v-if="listShow"  v-loading='loading'>
...
<script lang='ts'>
  ...
  export default {
    ...
    setup(props,{ emit }) {
      ...
      let listShow: Ref<boolean> = ref(false)
      const typeChange = (e) =>{
          if(e == 0){
              listShow = false
          }else if(e == 1){
      ...
      let selectId: string = params.value.subjectId
      //全部课程
      let courseList = ref([])
      ...
      const request = async (params?) => {
        loading.value = true;
        let res = await axios.post<any,AxResponse>('/course/queryByPage', {...params, current: page.current, size: page.size, selectId: selectId }, { headers: { type: 1, 'Content-Type': 'application/json'}});
        if (res.result) {
      ...
      let nearParams = {
            ...
            current: 1,
            size: 10,
            selectId: params.value.subjectId 
        }
      const nearRequest = async (params?) => {
        nearLoading.value = true;
        
        let res = await axios.post<any,AxResponse>('/admin/prepareLesson/queryPage',{...nearParams, current: page.current, size: page.size, selectId: selectId},{ headers: { type: 1, 'Content-Type': 'application/json'}})
        if (res.result) {
            page.total = res.json.total;
      ...
      return { headerRef, params, request, loading, courseList, courseDatials, nearRequest, nearLoading, typeChange, listShow }
    }
  }
</script>
