    ...
    setup(props,{ emit }) {
      ...
      let nearParams = {
        ...
        const nearRequest = async (params?) =>{
            let res = await axios.post<any,AxResponse>('/admin/prepareLesson/queryPage',{ ...nearParams, current: page.current, size: page.size, subjectId: subjectId},{ headers: { type: 1, 'Content-Type': 'application/json'}});
            if (res.result) {
                page.total = res.json.total;
