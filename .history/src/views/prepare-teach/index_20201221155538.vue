              ...
              <ul>
                <li v-for="item in courseDatials" :key="item">
                    <div class="near-list-left" >  
                        ...
                        <!-- <el-tooltip class="item" effect="dark" :content="item.courseIndexName" placement="bottom">
  ...
  export default {
    ...
    setup(props,{ emit }) {
      ...
      const request = async (params?) => {
        loading.value = true;
        let res = await axios.post<any,AxResponse>('/course/queryByPage', { ...params, current: page.current, size: page.size, }, { headers: { type: 1, 'Content-Type': 'application/json'}});
        if (res.result) {
      ...
      const nearRequest = async (params?) => {
        ...
        let res = await axios.post<any,AxResponse>('/admin/prepareLesson/queryPage',{ ...nearParams, current: page.current, size: page.size, },{ headers: { type: 1, 'Content-Type': 'application/json'}})
        if (res.result) {
            page.total = res.json.total;
