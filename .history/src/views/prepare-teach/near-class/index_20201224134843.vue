  ...
  <div class="near-cus-list">
    <cus-list has-page url="/admin/prepareLesson/queryPageV2" :default="params" :auto-request="true" :headers='{ type: 1 }'>
      ...
      <template v-slot:actions="{ data }">
        <div class="menu">
          <el-button size="small" v-if="item.checkStaus == 1"  @click="courseDetailFileList(data,index)">继续备课</el-button>
          <el-button size="small" v-if="item.checkStaus == 2"  @click="courseDetailFileList(data,index)">查看备课</el-button>
        </div>  
      </template>
    </cus-list>
  </div>
