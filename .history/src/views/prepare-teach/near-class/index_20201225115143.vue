...
export default {
  ...
  setup(props) {
    ...
    const prepareLessonRequest = new Promise((resolve, reject, id?) => {
        axios.post<any,AxResponse>('/admin/prepareLesson/queryPrepareLessonByCourseIndexId', { courseIndexId: id}).then(res => {
          if(res.result){
            console.log(res)
            resolve(res)
          }
    ...
    const savePrepareClass = async(data) => {
      Promise.all([prepareLessonRequest(data.courseIndex)]).then(res => {
        console.log(res)
      })
        prepareLessonRequest(data.courseIndex).then(res => {
          console.log(res,111)
        })
