  ...
  <div class="near-cus-list">
    <cus-list has-page url="/admin/prepareLesson/queryPageV2" :default="params" :auto-request="true" :headers='{ type: 1 }'>
      ...
      <template v-slot:actions="{ data }">
        <div class="menu">
          <el-button size="small" round :class="data.checkStaus === 2?'btn-hidden':''"  @click="courseDetailFileList(data,index)">提交备课</el-button>
          <el-button size="small" round type='primary' v-if="data.checkStaus === 1"  @click="courseDetailFileList(data,index)">继续备课</el-button>
          <el-button size="small" round type='primary' v-if="data.checkStaus === 2"  @click="courseDetailFileList(data,index)">查看备课</el-button>
        </div>  
      </template>
    </cus-list>
  </div>
...
export default {
  ...
  setup(props) {
    ...
    return { params, courseDetailFileList }
  }
}
</script>
