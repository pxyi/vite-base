<template>
  <div class="near-cus-list">
    <cus-list has-page url="/admin/prepareLesson/queryPageV2" :default="params" :auto-request="autoRequest" :headers='{ type: 1 }'>
...
export default {
  ...
  setup(props) {
    ...
    let autoRequest  = ref(false)
    console.log(autoRequest.value)
    ...
    const prepareLessonRequest = async(id?) => {
      let res = await axios.post<any,AxResponse>('/admin/prepareLesson/queryPrepareLessonByCourseIndexId', { courseIndexId: id})
      if(res.result){
    ...
    const savePrepareClass = async(data) => {
      prepareLessonRequest(data.courseIndex)
      let __params = {
    ...
    return { params, courseDetailFileList, savePrepareClass, autoRequest }
  }
}
</script>
