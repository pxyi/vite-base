...
export default {
  ...
  setup(props) {
    ...
    let allFileList = ref([])
    const request = (type) =>{
      let res = axios.post<any,AxResponse>('/admin/prepareLesson/queryMaterialByCourseIndexId', { courseIndexId: props.id, type })
      console.log(res,123)
    }
    request('')

    return { close, activeName, tabCountList, courseDto, request }
  }
}
</script>
