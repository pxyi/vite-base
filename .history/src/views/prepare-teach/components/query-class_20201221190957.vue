<template>
  <div class="cus__query__class__container" v-if="searchShow == 1">
    ...
    <div class="cus__class__item">
      ...
      <div class="cus__class__box">
        <div class="cus__class__cell" v-for="cell in searchRules.semesterId" :key="cell.id" @click="setQueryValue('terms', cell.id)" :class="{ active: cell.id === queryForm.semesterId }">{{ cell.name }}</div>
      </div>
    </div>
    <div class="cus__class__item">
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const getRules = async (subjectId) => {
      ...
      emitter.emit('queryClass', { years:  res.json.years, grades: res.json.grades, semesterId: res.json.semesterId, courseTypes: res.json.courseTypes })
      res.json.years = [ { name: '全部', id: null }, ...res.json.years ];
      res.json.grades = [ { name: '全部', id: null }, ...res.json.grades ];
      res.json.semesterId = [ { name: '全部', id: null }, ...res.json.semesterId ];
