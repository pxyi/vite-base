  ...
  <div class="paper__update__container">
    <div class="header">
      ...
      <div class="btns">
        ...
        <el-button round @click="savePrepareClass;close()" :disabled="prepareLesson == '' && prepareLesson.id == null " v-if="!prepareLesson.checkStatus || prepareLesson.checkStatus === 0">提交备课</el-button>
        <el-button round v-if="prepareLesson.checkStatus == 1"  >已提交</el-button>
...
export default {
  ...
  setup(props) {
    ...
    // 获取上部展示数据
    let courseDto = ref({})
    let prepareLesson = ref({})
    const prepareLessonRequest = async() => {
      let res = await axios.post<any,AxResponse>('/admin/prepareLesson/queryPrepareLessonByCourseIndexId', { courseIndexId: props.id})
      if(res.result){
    ...
    // 上传我的教案
    const uploadMyPlan = () => {
      Modal.create({ title: '上传我的教案', width: 640, component: MyPlanUpload, props: { id: props.id }, zIndex: 999 })
    }

    // 上传我的说课
    const uploadMyVideo = () => {
      Modal.create({ title: '上传我的说课', width: 640, component: MyVideoUpload, props: { id: props.id }, zIndex: 999 })
    }

    // 提交备课
    const savePrepareClass = async() => {
      let __params = {
    ...
    return { close, activeName, tabCountList, courseDto, request, allFileList, handleClick, prepareLesson, uploadMyPlan, prepareLessonRequest, tabCountRequest, uploadMyVideo }
  }
}
</script>
