<template>
  <div class="cus__query__class__container" v-if="searchShow == 1">
    ...
    <div class="cus__class__item">
      ...
      <div class="cus__class__box">
        <div class="cus__class__cell" v-for="cell in searchRules.terms" :key="cell.id" @click="setQueryValue('semesterId', cell.id)" :class="{ active: cell.id === queryForm.terms }">{{ cell.name }}</div>
      </div>
    </div>
    <div class="cus__class__item">
      <div class="cus__class__label">班型</div>
      <div class="cus__class__box">
        <div class="cus__class__cell" v-for="cell in searchRules.courseTypes" :key="cell.id" @click="setQueryValue('courseTypeId', cell.id)" :class="{ active: cell.id === queryForm.courseTypes }">{{ cell.name }}</div>
      </div>
    </div>
  </div>
</template>
