  ...
  <div class="cus__query__class__container" v-if="searchShow == 1">
    <div class="cus__class__item">
      ...
      <div class="cus__class__box">
        <div class="cus__class__cell" v-for="cell in searchRules.year" :key="cell.id" @click="setQueryValue('year', cell.id)" :class="{ active: cell.id === queryForm.year }">{{ cell.name }}</div>
      </div>
    </div>
    <div class="cus__class__item">
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const getRules = async (subjectId) => {
      ...
      emitter.emit('queryClass', { years:  res.json.year, grades: res.json.grades, terms: res.json.terms, courseTypes: res.json.courseTypes })
      res.json.year = [ { name: '全部', id: null }, ...res.json.year ];
