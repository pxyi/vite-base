  ...
  export default {
    ...
    setup(props,{ emit }) {
      ...
      const request = async (params?) => {
        loading.value = true;
        let res = await axios.post<any,AxResponse>('/course/queryByPage', { ...params, current: page.current, size: page.size, subjectId: params.value.subjectId }, { headers: { type: 1, 'Content-Type': 'application/json'}});
        if (res.result) {
      ...
      let nearParams = {
        ...
        const nearRequest = async (params?) =>{
            let res = await axios.post<any,AxResponse>('/admin/prepareLesson/queryPage',{ ...nearParams, current: page.current, size: page.size, subjectId: params.value.subjectId},{ headers: { type: 1, 'Content-Type': 'application/json'}});
            if (res.result) {
                page.total = res.json.total;
                courseDatials.value = res.json.records;
                console.log(courseDatials.value,111)
            }
        }
