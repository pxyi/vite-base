<template>
  ...
  <div>
    <QueryClassComponent @query="$refs.cusList.request($event)"/>
    <div>
      ...
      <div class="cus-list" v-if="params.value == 0">
        <cus-list ref="list" has-page url="/teachbook/Courseware/queryPage" :default="params" :auto-request="false">
  ...
  export default {
    ...
    setup(props,{ emit }) {
      ...
      const request = async (params?) => {
        ...
        // loading.value = true;
        // let res = await axios.post<any, AxResponse>('', { ...props.default, ...__params, current: page.current, size: page.size });
        // if (res.result) {
        //   page.total = res.json.total;
        //   cusList.value = res.json.records;
        // }
        loading.value = false;
      }

      return { headerRef, params, courseList, equest }
    }
  }
</script>
