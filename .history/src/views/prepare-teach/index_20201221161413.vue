  ...
  export default {
    ...
    setup(props,{ emit }) {
      ...
      const request = async (params?) => {
        loading.value = true;
        let res = await axios.post<any,AxResponse>('/course/queryByPage', { current: page.current, size: page.size, selectId:params.value.selectId}, { headers: { type: 1, 'Content-Type': 'application/json'}});
        if (res.result) {
