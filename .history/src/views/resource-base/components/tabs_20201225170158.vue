...
export default {
  ...
  const getMaterialQueryPage = (item?)=>{
    ...
    // console.log(item.type);
     axios.post<any,AxResponse>(`admin/material/queryPage?size=${20}&current=${1}`,params,   { headers: { "Content-Type": "application/json" } }).then(res=>{
       if(res.result){
    ...
    let renameDialog:Boolean = false
      ...
      let {fileName} = item
      Modal.create({ title: '重命名', width: 480, component: newName, props: {contengList} });
    }
      const downLoad = item=>{
      console.log(item);
